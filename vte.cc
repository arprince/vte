...
//make sure to add to .h
void
Terminal::scroll_up_to_command()
{
	_vte_debug_print(VTE_DEBUG_ADJ,
			"entered scroll up\n");
	//more accurate username https://unix.stackexchange.com/questions/157426/what-is-the-regex-to-validate-linux-users 
	//hostname info https://stackoverflow.com/questions/3523028/valid-characters-of-a-hostname
	auto regex = vte_regex_new_for_search("4.4", -1, 0, 0); //Regex will pick up any word at start of string with @ in middle and ending in :
...
Terminal::scroll_down_to_command()
{
	...
	auto regex = vte_regex_new_for_search("4.4", -1, 0, 0); //^.+@.+:   sh-4.4$
	search_set_regex(regex, 0);
